...
# For reference see https://docs.netpoint.dev/en/stable/configuration/
# Based on https://github.com/khulnasoft/netpoint/blob/develop/netpoint/netpoint/configuration_example.py

###
# NetPoint-Docker Helper functions
###
...
DATABASE = {
    'NAME': environ.get('DB_NAME', 'netpoint'),       # Database name
...
# JOBRESULT_RETENTION was renamed to JOB_RETENTION in the v3.5.0 release of NetPoint. For backwards compatibility, map JOBRESULT_RETENTION to JOB_RETENTION
...
# Set this to True only if you are actively developing the NetPoint code base.
...
# are permitted to access most data in NetPoint (excluding secrets) but not make any changes.
...
REMOTE_AUTH_BACKEND = _environ_get_and_map('REMOTE_AUTH_BACKEND', 'netpoint.authentication.RemoteUserBackend', _AS_LIST)
