#!/bin/bash
# Clones the NetPoint repository with git from Github and builds the Dockerfile
...
SRC_ORG     Which fork of netpoint to use (i.e. github.com/\${SRC_ORG}/\${SRC_REPO}).
            ${_GREEN}Default:${_CLEAR} khulnasoft
...
IMAGE_NAMES The names used for the image including the registry
            ...
            ${_GREEN}Default:${_CLEAR} docker.io/khulnasoft/netpoint
            ${_CYAN}Example:${_CLEAR} 'docker.io/khulnasoft/netpoint quay.io/khulnasoft/netpoint'
...
BUILDX_REMOVE_BUILDER
            ...
            This is useful if you build NetPoint Docker on an automated system that does
...
IMAGE_NAMES="${IMAGE_NAMES-docker.io/khulnasoft/netpoint}"
...
gh_echo "::group::⤵️ Fetching the NetPoint source code"

###
# Variables for fetching the NetPoint source
###
...
# - base image digest
# - netpoint git ref (Label: netpoint.git-ref)
# - netpoint-docker git ref (Label: org.opencontainers.image.revision)
###
...
if [ -z "${GH_ACTION}" ]; then
  ...
  if [ "${GIT_REF}" != "${GIT_REF_OLD}" ]; then
    SHOULD_BUILD="true"
    BUILD_REASON="${BUILD_REASON} netpoint-docker"
  fi
fi
...
# --label
DOCKER_BUILD_ARGS+=(
  --label "netpoint.original-tag=${TARGET_DOCKER_TAG_PROJECT}"
...
if [ -n "${BUILD_REASON}" ]; then
  ...
  DOCKER_BUILD_ARGS+=(--label "netpoint.build-reason=${BUILD_REASON}")
  DOCKER_BUILD_ARGS+=(--label "netpoint.last-base-image-layer=${BASE_LAST_LAYER}")
fi
