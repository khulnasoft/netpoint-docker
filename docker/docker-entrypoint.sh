#!/bin/bash
# Runs on every start of the NetPoint Docker container

# Stop when an error occures
set -e

# Allows NetPoint to be run as non-root users
umask 002
...
while [ "${CUR_DB_WAIT_TIME}" -lt "${MAX_DB_WAIT_TIME}" ]; do
  ...
  if [ -n "$DB_WAIT_DEBUG" ]; then
    ...
    echo "${DB_ERR_LINES[@]: -1}"
    echo "[ Use DB_WAIT_DEBUG=1 in netpoint.env to print full traceback for errors here ]"
  fi
